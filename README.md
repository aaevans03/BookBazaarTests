...
- Then, create a `apiConfig.js` file in the repo root directory. Replace `CUSTOM-URL` with the URL for the `mockapi.io`.

```js
// apiConfig.js
module.exports = {
    apiUrl: 'https://CUSTOM-URL.mockapi.io'
};
```
...
## Tests implemented
1. `test("Happy path test: POST/GET a new book")`: POST a new book to the endpoint, GET the list of books and verify the new book is in it.
1. `test("Edge/validation case: POST with invalid format")`: POST an invalid book to the endpoint, expect an error.
...
1. `test("Negative/error case: DELETE nonexistent book")`: DELETE a nonexistent book, expect an error.
